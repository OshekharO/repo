...
// @lang         en
// @license      MIT
// @icon         https://asianc.id/template/images/logo.jpg
...
export default class extends Extension {
  ...
  async load() {
    this.registerSetting({
      title: "Dramacool Site URL",
      key: "dramacool",
      type: "input",
      description: "Dramacool Site Url",
      defaultValue: "https://asianc.id",
    });
  }

  initCryptoJS() {
    if (this.CryptoJS) return;
var CryptoJS = CryptoJS||function(u,p){var d={},l=d.lib={},s=function(){},t=l.Base={extend:function(a){s.prototype=this;var c=new s;a&&c.mixIn(a);c.hasOwnProperty("init")||(c.init=function(){c.$super.init.apply(this,arguments)});c.init.prototype=c;c.$super=this;return c},create:function(){var a=this.extend();a.init.apply(a,arguments);return a},init:function(){},mixIn:function(a){for(var c in a)a.hasOwnProperty(c)&&(this[c]=a[c]);a.hasOwnProperty("toString")&&(this.toString=a.toString)},clone:function(){return this.init.prototype.extend(this)}},
    ...
    for (const li of lis) {
      ...
      if (hrefMatch && imgMatch && titleMatch) {
        ...
        if (slug.includes("episode-")) {
          slug = "drama-detail/" + slug.replace(/-episode-\d+\.html$/, "");
        }
  ...
  async detail(url) {
    ...
    const descMatch = res.match(/<p[^>]*class="desc"[^>]*>([\s\S]*?)<\/p>/) || res.match(/<p[^>]*>([\s\S]*?)<\/p>/);
  ...
  async watch(url) {
    ...
    if (res) {
      ...
      if (directUrlMatch) {
        ...
        const iframeMatch = res.match(/<iframe[\s\S]*?src="([^"]+)"/i) || res.match(/data-video="([^"]+)"/i);
        if (iframeMatch) {
          ...
          try {
            ...
            let thirdIframeMatch = embedRes.match(/<iframe[\s\S]*?src="([^"]+)"/i);
            let thirdUrl = thirdIframeMatch ? thirdIframeMatch[1].trim() : "";
            if (thirdUrl) {
              if (thirdUrl.startsWith("/")) {
                const uObj = new URL(embedUrl);
                thirdUrl = uObj.origin + thirdUrl;
              }

              const thirdRes = await this.request("", {
                headers: {
                  "Miru-Url": thirdUrl,
                  Referer: embedUrl,
                  "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
              ...
              const cryptoMatch = thirdRes.match(/data-value="([^"]+)"/);
              if (cryptoMatch) {
                this.initCryptoJS();
                const key = this.CryptoJS.enc.Utf8.parse("94588293375053432799222445521289");
                const iv = this.CryptoJS.enc.Utf8.parse("5259228356829423");
                const decrypted = this.CryptoJS.AES.decrypt(cryptoMatch[1], key, {
                  iv: iv,
                  mode: this.CryptoJS.mode.CBC,
                  padding: this.CryptoJS.pad.Pkcs7,
                });
                directUrl = decrypted.toString(this.CryptoJS.enc.Utf8);
              }
            }
          } catch (e) {
            // ignore
